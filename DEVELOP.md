...
| 2026-09-22 | **动态跨模态哈希检索完整落地（模拟数据阶段）**：① 新增模拟多模态语料生成器（300 记录 × 文本/图像/语音三视图，词汇互不重叠，含主题/风险/时间窗标注，写入 `backend/data/hashing/corpus.db`）；② 索引层重写为「时间窗 × 位带」多哈希表（64 位码切 8 位带，位带内探测 2 比特 → 汉明半径 16 内不漏召回；每窗口独立评估 σ 信息量 / ρ 语义一致性，弱表淘汰）；③ 引擎改为**单元级索引**（记录×模态视图为单元，共享同一码）+ 配对三模态训练 + **非对称距离排序**（查询端连续向量 · 库端 ±1 码）+ explain 可解释字段；④ 特征改为「TF 哈希桶 ⊕ 主题语义向量」复合特征（跨模态共享语义桥，权重 0.35）；⑤ 新增 `scripts/demo_hashing.py`（一键演示）、`scripts/eval_retrieval.py`（mAP/Recall/随机基线/权重扫描/码空间诊断）；⑥ 前端检索页新增检索范围筛选、跨模态与"为什么相似"展示，停用未接通的语音/图像上传按钮 | backend/scripts/generate_retrieval_corpus.py、backend/scripts/demo_hashing.py、backend/scripts/eval_retrieval.py（新增），backend/app/engines/hashing/{engine,multi_table_index,cmfh,features,mock_engine}.py、backend/app/config.py、backend/app/handlers/{retrieval,personal,admin}.py、frontend/src/{pages/Retrieval.tsx,services/mockApi.ts,data/mockData.ts} | 实测（合成语料，80 条查询）：mAP@10 0.844，随机基线 0.196（提升 4.29×），跨模态目标 mAP 图像 0.503 / 语音 0.519，平均检索 4.5ms。**口径限制**：语料为模板生成、相关性由主题重叠定义，指标只用于链路验证与相对比较，不能作为论文绝对结论；真实性能需接入真实配对多模态数据后重测 |
| 2026-09-22 | 检索结果脱敏与返回契约统一：引擎新增 `deidentify()`，命中项统一经 `to_frontend_hit()` 输出（camelCase + explain），学生端/管理端/分析端共用；新增 `GET /api/retrieval/index-stats`（索引规模、码长、时间窗表健康度） | backend/app/engines/hashing/features.py、backend/app/handlers/retrieval.py、backend/app/handlers/__init__.py | 仍待办：`screenings` 表缺 `user_id`，学生端检索尚未做归属作用域过滤（需与账号模块对齐后实现） |
| 2026-09-22 | 哈希引擎状态文件治理：状态版本升到 3 并校验特征维度（维度变化自动重建，避免"全零码"静默失效）；状态 JSON 精简（去掉重复文本字段，1.49MB → 1.29MB）；`hashing_state.json` 加入 .gitignore、`corpus.db` 放行入库 | backend/app/engines/hashing/engine.py、backend/.gitignore、backend/.env.example | 已跟踪的旧状态文件需 `git rm --cached backend/data/hashing/hashing_state.json` 后提交 |
