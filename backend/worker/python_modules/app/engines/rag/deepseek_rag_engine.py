    ...
    async def generate_report(self, screening_data: Dict) -> Dict:
        ...
        try:
            ...
            if runtime.is_worker():
                # Worker（Pyodide）无线程、且 Pyodide 无 socket：走 JS fetch 异步调用
                content = await self._call_deepseek_worker(prompt)
            else:
                # 本地：阻塞式 HTTP 放到线程，避免卡住事件循环
...
请直接返回JSON，不要包含其他文字。"""
    ...
    async def _call_deepseek_worker(self, prompt: str) -> str:
        """Worker（Pyodide）下的网络调用。

        Pyodide 无 socket，urllib 不可用于外发请求；改用 Cloudflare 运行时
        内置的 JS fetch 发起异步 HTTP 请求，payload 与本地 _call_deepseek 完全一致。
        """
        ...
        # Pyodide 通常把 JS 对象自动转为 Python dict；若返回 PyProxy 则手动转换
